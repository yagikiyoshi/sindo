!---+----1----+----2----+----3----+----4----+----5----+----6----+----7----+----80
   ...
   Character(len=32) HostName
      ...
      Write(Iout,*) '    ***                (  VERSION 3.7  PADDINGTON  ) '
      Write(Iout,*) '     ***                                             '
      Write(Iout,*) '      ***       COPYRIGHT 2015:                      '
...
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7----+----80
      ...
      subroutine chlng(buf,nmj)
! **************************************************
! *  obtaining the length of characters            *
! *    coded by Naoki MAEDA                        *
! **************************************************
      character buf*(*)
      n2=len(buf)
      do 100 i=n2,1,-1
      if(buf(i:i).ne.' ') go to 110
  100 continue
      nmj=0
      return
  110 nmj=i
...
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7----+----80
!
      subroutine hditem(buf,hd,nhd,nhdmx)
! *************************************************
! *  spliting charcter string at space or comma.  *
! *        coded by Naoki MAEDA on 2000/10/28     *
! *  TAB(char(9)) code is added as a separator    *
! *        modified on 2001/08/15                 *
! *  nhdmx is added                               *
! *        modified on 2003/09/24                 *
! *************************************************
      character buf*(*)
      CHARACTER HD(1)*(*)
! 
...
! ** space **
      if(buf(i:i).eq.' ') then
        if(ispc.eq.0.and.icom.ne.1) then
            ispc=1
            ihd=ihd+1
            hd(ihd)=' '
            m=0
        ...
        icom=0
! ** comma **
      else if(buf(i:i).eq.','.or.buf(i:i).eq.char(9)) then
         if(ispc.eq.0.or.icom.eq.1) then
            ihd=ihd+1
            hd(ihd)=' '
            m=0
         end if
...
!---+----1----+----2----+----3----+----4----+----5----+----6----+----7----+----80
      ...
      subroutine split(buf,spc,hd,nhd,nhdmx)
! *************************************************
! *  spliting charcter string at separator.       *
! *        coded by Naoki MAEDA on 2002/10/16     *
! *  nhdmx is added                               *
! *        modified on 2003/09/24                 *
! *************************************************
      character buf*(*),spc*1
